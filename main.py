...
# 打印配置状态（不显示敏感信息）
print(f"🔧 配置检查:")
print(f"   - SERPER_API_KEY: {'✅ 已设置' if SERPER_API_KEY else '❌ 未设置'}")
print(f"   - DEEPSEEK_API_KEY: {'✅ 已设置' if DEEPSEEK_API_KEY else '❌ 未设置'}")
print(f"   - WXPUSHER_APP_TOKEN: {'✅ 已设置' if WXPUSHER_APP_TOKEN else '❌ 未设置'}")
print(f"   - WXPUSHER_UID: {USER_UID}")
...
# ==================== AI 总结 ====================
def summarize_with_openai(news_list: List[Dict], max_items: int = 8) -> str:
    """
...
"""
    ...
    # 构建 Prompt
    prompt = f"""你是一位专业的新闻分析师。请阅读以下过去24小时的重要新闻。

任务要求：
1. 筛选出最值得关注的 {max_items} 条新闻
2. 新闻分类包括：科技/科学/商业新闻 + 伊朗/中东战争/地缘政治新闻
3. 每条新闻提炼一个核心要点（不超过30字）
4. 必须保留原始新闻链接

输出格式（严格使用以下HTML格式，分两个板块）：

<h3 style="color: #333; margin-top: 20px;">🔬 科技·科学·商业</h3>
...
<h3 style="color: #333; margin-top: 20px;">⚔️ 伊朗·中东局势</h3>
<div style="margin-bottom: 15px; padding: 10px; background: #fff0f0; border-radius: 5px;">
    <strong style="color: #c62828;">📰 新闻标题</strong><br/>
    <span style="color: #666;">💡 核心要点</span><br/>
    <a href="链接" style="color: #c62828;">查看原文 →</a>
</div>
...
def push_to_wxpusher(content: str) -> bool:
    ...
    data = {
        ...
        "content": content,
        "summary": f"📰 {datetime.date.today()} 每日新闻简报（科技+中东）",
        "contentType": 2,  # HTML 内容
        "uids": [USER_UID]
    }
    
    print(f"📱 推送参数:")
    print(f"   - appToken: {WXPUSHER_APP_TOKEN[:10]}..." if WXPUSHER_APP_TOKEN and len(WXPUSHER_APP_TOKEN) > 10 else f"   - appToken: {'已设置' if WXPUSHER_APP_TOKEN else '未设置'}")
    print(f"   - uid: {USER_UID}")
    print(f"   - contentType: 2 (HTML)")
...
def main():
    ...
    # 2. 获取新闻
    # 第一类：科技商业科学新闻
    news_list_tech = get_news_from_serper(
        keywords="科技 商业 科学 重大新闻",
        num=10
    )
    
    # 第二类：伊朗及中东战争最新新闻
    news_list_iran = get_news_from_serper(
        keywords="伊朗 中东战争 最新 战况",
        num=8
    )
    
    # 合并两类新闻
    news_list = news_list_tech + news_list_iran
